    ...
    try {
        ...
        while (true) {
            ...
            if (options.maxPages && clickCount >= options.maxPages - 1) {
                console.log(`ℹ️  [${providerKey}] Reached maxPages limit of ${options.maxPages}. Stopping page loads.`);
                break;
            }
        ...
        fs.writeFileSync(outputPath, JSON.stringify(finalUrls, null, 2), 'utf-8');
        console.log(`✅ [${providerKey}] Saved ${finalUrls.length} links to: ${outputPath}`);
    } catch (error: any) {
